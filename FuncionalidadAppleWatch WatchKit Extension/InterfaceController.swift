...
//
//  Created by user191401 on 11/28/21.
//
...
class InterfaceController: WKInterfaceController {
    ...
    func updateGUI(lista_user: [users]){
        DispatchQueue.main.async {
            ...
            self.theTable.setNumberOfRows(self.datos.count, withRowType: "lines")
            for index in 0 ..< self.datos.count {
                let theLine = self.theTable.rowController(at: index) as! TableController
